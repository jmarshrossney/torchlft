...
class TransformModule(nn.Module):
    def forward(self, ϕ_passive: Tensor):
        params = self.conditioner(ϕ_passive)
        return self.transform(params)

class AffineModule(TransformModule):
    ...
...
class CouplingFlow(nn.Module):
    def __init__(
        ...
        n_params = transform_cls.n_params


class CouplingFlow(nn.Module):
    def __init__(self, geometry: Geometry, transforms: list[TransformModule]):
        assert len(transforms) % 2 == 0
    ...
    def forward(self, ϕ: Tensor) -> tuple[Tensor, Tensor]:
        ϕ_a, ϕ_b = self.geometry.split(ϕ)

        for layer in range(len(self)):
            f_a = transform_a(ϕ_b)
            ϕ_a, diag_jac_a = f_a(ϕ_a)

            f_b = transform_b(ϕ_a)
            ϕ_b, diag_jac_b = f_b(ϕ_b)

            log_diag_jac += diag_jac_a.log() + diag_jac_b.log()

        ϕ = self.geometry.join(ϕ_a, ϕ_b)
        log_det_jac = sum_except_batch(log_diag_jac)

        return ϕ, log_det_jac
